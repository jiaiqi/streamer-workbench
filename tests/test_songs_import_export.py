...
def _song(title, **over):
    _id_counter[0] += 1
    base = {"id": f"song_test_{_id_counter[0]:04d}", "title": title, "artists": [], "lyricist": "", "composer": "",
        ...
        async def scenario():
            with tempfile.TemporaryDirectory() as raw:
                ...
                async with app.router.lifespan_context(app):
                    ...
                    # 默认 status 是 draft；3 首歌：晴天 跳过、夜曲 add draft、稻香 add draft
                    self.assertEqual(payload["active"], 1)
                    self.assertEqual(payload["draft"], 2)
                    self.assertEqual(payload["errors"], [])
        asyncio.run(scenario())

    def test_import_merge_explicit_active(self):
        """merge 模式 + 显式 status=active → 新增 2 首 active"""
        async def scenario():
            with tempfile.TemporaryDirectory() as raw:
                data_root = Path(raw)
                _write_songs_json(data_root, {"version": 5, "songs": [_song("晴天")]})
            ...
            with tempfile.TemporaryDirectory() as raw:
                ...
                async with app.router.lifespan_context(app):
                    status, body, _ = await _raw_request(
                        app, "POST", "/api/songs/import",
                        {"mode": "replace", "songs": [
                            {"title": "新歌A", "status": "active"},
                            {"title": "新歌B", "status": "active"},
                        ]})
                    self.assertEqual(status, 200, body)
                    payload = json.loads(body)
                    self.assertEqual(payload["added"], 2)
