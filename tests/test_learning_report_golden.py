"""R3.5 learning-report 新金标准测试。

策略（与 magazine-flow / live-set 一致）：
- 不做 pixel diff（避免引擎自举）
- 验证 PNG 文件存在 + manifest 中 sha256 与文件实际一致
- 5 个 case：empty / month / active / keyed / difficult

约束：
- 旧 16 张 grid-wrap + 6 张 magazine-flow + 5 张 live-set 金标准不被本测试触碰
- 重生成需用 `tools/generate_learning_report_golden.py --confirm-baseline`
"""
...
GOLDEN_DIR = PROJECT_ROOT / "tests" / "golden_learning_report"
MANIFEST = GOLDEN_DIR / "manifest.json"


class LearningReportGoldenTests(unittest.TestCase):
    ...
    def test_manifest_lists_five_cases(self):
        """5 个代表性用例：empty / month / active / keyed / difficult。"""
        data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    ...
    def test_engine_metadata(self):
        ...
        self.assertEqual(data["engine"], "learning-report-r35")
