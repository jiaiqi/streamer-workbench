...
### 产品优化方案终版：产品、UI/UX、架构与多布局升级总线

> **唯一主规格（必读）**：`design/产品优化方案终版-0727/产品优化方案终版.md`——三套前序方案合并并按当前代码事实纠偏后的最终产品、UI/UX、架构、多布局、自定义编辑器、Electron 与发布质量规格。
> **实施路线图**：`design/产品优化方案终版-0727/路线图.md`——P0–P9 依赖、阶段退出条件、S1–S5 合流、并行支线和发布验收。
> **差异裁决**：`design/产品优化方案终版-0727/三方案差异分析.md`——v1/v2/v3 的保留、舍弃与冲突处理。前序方案保留为历史，不再作为并行执行规格。
> **执行关系**：当前数据线可继续完成下方 S4/S5；新产品主线从终版 P0 基线冻结开始，不跳过 Schema、兼容和金标准守护。

- [ ] **P0–P1 基线与模型**：事实基线、ADR、去魔数、Layout/Palette/Skin/Preset 兼容层、Preset 与服务端地基
- [ ] **P2–P4 多布局系统**：Shape/路径/碰撞/分页运行时、8 套核心布局、背景主体环绕
- [ ] **P5–P6 产品体验闭环**：工作台系统性重构、预设/主题/历史/统计和数据反哺创作
- [ ] **P7 桌面产品化**：正式 Electron 壳、用户目录、置顶速查、全局快捷键
- [ ] **P8–P9 用户创造与发布质量**：数据容器式布局编辑器、性能/无障碍/数据恢复/正式发布门
