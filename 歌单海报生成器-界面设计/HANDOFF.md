...
- **歌曲**：内置 177 首（`core/data/songs.py`），**已补「奇妙能力歌」（目标 178）**。歌曲只有 title/status/section（1~6=字数, 7=长歌名/英文），artists/tags 等元数据为空。设计稿中「在学 5 首」是虚构演示数据。
...
3. ~~分组规则已定案~~（2026-07-25 已完成）
4. ~~**补数据**~~：已补「奇妙能力歌」→ 178 首。金标准 14/14 diff=0。
5. ~~`tools/migrate_data.py`~~：已完成。双源校验通过，已产出 data/songs.json（178 首）。
