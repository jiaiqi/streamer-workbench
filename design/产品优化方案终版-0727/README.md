# 主播工作台 · 产品优化方案终版（2026-07-28）

> **权威性声明**：本目录是三套前序优化方案合并、核验和裁决后的唯一执行主规格。
> 前序方案（方案1/2/3）已归档清理；差异裁决记录见 [`三方案差异分析.md`](三方案差异分析.md)。

## 文件导航

| 文件 | 用途 | 建议读者 |
|---|---|---|
| [`产品优化方案终版.md`](产品优化方案终版.md) | 产品、UI/UX、架构、多布局、自定义编辑器、桌面端和质量体系的完整规格 | 全体参与者 |
| [`三方案差异分析.md`](三方案差异分析.md) | v1/v2/v3 差异、重复、冲突、事实核验与终版裁决 | 产品负责人、架构师、评审者 |
| [`路线图.md`](路线图.md) | R0–R7 唯一执行路径：正确性收口、三个纵向闭环、运行时冻结和发布门 | 开发、测试、后续 Agent |

## 推荐阅读顺序

1. 首次接手：本文件 → `产品优化方案终版.md` §1–§3 → `路线图.md`；
2. 当前接手：先读路线图 R0、`ADR-003.md`、`ADR-004.md`；
3. 做布局：主规格 §5–§10 → 路线图 R1/R4/R6；
4. 做 UI/UX：主规格 §4、§9 → 路线图 R1/R5；
5. 做数据闭环：主规格 §11 → `../roadmap-data-stats.md` → 路线图 R0/R3；
6. 做桌面壳：主规格 §11.4 → 路线图 R2/R7；
7. 审计决策来源：`三方案差异分析.md`。

## 关联文档清单

以下文档**不在终版目录中**，但包含终版未重复的独特价值，必须保留：

| 文档 | 位置 | 终版未覆盖的独特价值 |
|---|---|---|
| **项目 README** | `../../README.md` | 开发环境运行命令、目录结构、金标准测试、API 列表、git 工作流 |
| **HANDOFF 交接文档** | `../../.archive/design-docs/歌单海报生成器-界面设计/HANDOFF.md` | 24 个坑位（终版覆盖 13 个）、开发验证命令、agent-browser 工作流、git log |
| **设计结论** | `../../.archive/design-docs/歌单海报生成器-设计结论.md` | 5 次技术选型推翻历史、palette.json/skin.json 具体 schema 草案、移动端判断 |
| **项目结构设计** | `../../.archive/design-docs/歌单海报生成器-项目结构设计.md` | Song 模型 17 字段完整定义、theme.json 预留字段、迁移映射表、14 条验收标准 |
| **方案 Review** | `../../.archive/design-docs/方案-review-2026-07-25.md` | 具体代码示例、CI yaml 样板、P2 级待办清单 |
| **数据时间维度路线图** | `../roadmap-data-stats.md` | S1–S5 事件类型、Song v5/Event v2 前置和统计口径唯一真相 |
| **用户数据 ADR** | `../../ADR-003.md` | 后端单一写入权威、平台目录和迁移边界 |
| **歌曲身份 ADR** | `../../ADR-004.md` | Song.id、Event v2、附件/队列/Preset 关联边界 |
| **grid-wrap 像素级规格** | `../../.archive/design-docs/歌单-排版一/歌单更新提示词.md` | grid-wrap 布局的分类规则、字号/间距/标签/柔光/避让区的像素级参数 |
| **设计系统 CSS** | `../../.archive/design-docs/歌单海报生成器-界面设计/shared.css` | 13 个 gp-* 组件类实现、暗色令牌覆盖、动效类、prefers-reduced-motion 降级 |
| **设计系统 JS** | `../../.archive/design-docs/歌单海报生成器-界面设计/shared.js` | gp.toast/applyTheme/toggleTheme、折叠面板/分段选择器自动初始化 |
| **设计令牌** | `../../.archive/design-docs/歌单海报生成器-界面设计/design-tokens.json` | 亮暗色板（24 语义色）、圆角、阴影、字体栈、缓动曲线——单源真值 |
| **7 页设计稿** | `../../.archive/design-docs/歌单海报生成器-界面设计/pages/*.html` | 高保真实现级设计稿（workspace/library/learning/themes/presets/history/settings） |
| **UI 重设计交互稿 v1** | `../redesign-v1.html` | 亮色 v1 方向完整 HTML/CSS/JS 实现 |
| **UI 重设计交互稿 v2** | `../redesign-v2.html` | 暗色"演出后台"方向，含统计视图指引 |
| **AI 生图提示词** | `../../.archive/design-docs/codex生图提示词/` | 制作新主题背景图的提示词模板 |
| **歌单分类参考** | `../../.archive/design-docs/歌单-排版一/歌单-字数分类-不带歌手.md` | 178 首歌按字数分类的人类可读列表 |
| **金标准预言机** | `../../.archive/design-docs/歌单-排版一/` | 独立预言机旧脚本 + 歌单数据，金标准重建来源 |

## 6 类终版缺口说明

终版是产品与技术规格，以下内容定位上不适合复刻到终版中，而是通过引用保留：

1. **开发实践坑位**：HANDOFF 坑 #2/#3/#4/#6/#7/#15/#23/#24 共 8 个，见 `产品优化方案终版.md` 附录 A
2. **设计系统实现**：13 个 gp-* 组件类清单、防 FOUC 脚本模板，见 shared.css / shared.js
3. **Song 模型完整字段**：17 字段定义，见 `项目结构设计.md` 或 `core/data/songs.py`
4. **theme.json 预留字段**：palette_ref/source_image/featured_artists，见 `项目结构设计.md`
5. **S4/S5 事件 Schema 与统计口径**：见 `../roadmap-data-stats.md`
6. **grid-wrap 像素级排版规格**：见 `../../.archive/design-docs/歌单-排版一/歌单更新提示词.md`

## 当前代码事实快照

以下是**当前状态**，不是目标状态：

- 7 套主题，但只有 `grid-wrap` 一个真实布局；
- 画布 UI 只有 1080×1920 和 1080×2400 两个预设；
- 金标准 16/16 diff=0；
- 歌曲数据 v4，共 178 首，状态仅 `active/draft` 两态；
- S1 事件日志、S2 点歌双写、S3 曲谱管理已上线；S4 学歌打卡、S5 统计视图未完成；
- Electron/PyInstaller 仅完成 spike，R2 Desktop Beta 与 R7 正式壳未完成；
- Palette、Skin、Preset 已有领域模型骨架，但未完整接入渲染和 UI；anchors、FreeformLayout、自定义编辑器未实现；
- React 工作台可用，但暗色令牌、组件体系、动效、响应式和无障碍仍需系统升级；
- benchmark 工具已建立，当前机器热渲染约 42–50ms；正式预算尚未冻结；
- deps + routers 已拆分；仍有业务全局引用、CORS 全开、统一错误和数据目录缺口；
- 当前唯一活跃阶段是 R0，不允许直接跳到 S4/S5 或完整通用布局运行时。

## 核心结论

> 不整项目重写；保留 PIL 引擎、数据层、FastAPI、React、插件思想和金标准。重构产品表达层、布局运行时、主题/皮肤边界、工作台状态与桌面产品化能力。

*本目录建立后，新增产品级决策应优先修改终版，不再分别维护前序方案。*
