...
> 最后更新：2026-09-06（P1 阶段二 4 刀：① R5c WCAG AA 颜色审计修复 + stagger reduce-motion 兜底（亮 muted-fg 5.19 / warning 5.31 / danger 5.50；新增 --color-danger-text + --color-accent-text token + ui/src/design-tokens.a11y.test.tsx 8 类对比度契约）② P1-A1.1 今晚动线升格首屏 — views/TonightView.tsx 复用 TonightWorkbench 5 区 + 默认 view 'workspace'→'tonight' + navItems 调整 + Icon.tonight + 命令面板增 view-tonight + 1 项 App.react.test.tsx 回归修复 + 3 项新 TonightView 容器层 spec）③ P1-A1.2 步骤 1 就绪度共享 lib — ui/src/lib/readiness.ts 5 字段最小窄类型 + evaluateReadiness/buildReadinessChips/aggregateReadiness 等纯函数 + 12 项 readiness 单测）④ P1-A1.2 步骤 2 ReadinessBadge 组件 + LibraryView 行内接入 — components/ReadinessBadge.tsx 4 枚徽章 xs/sm 两档 + 暗/亮两色 + data-testid/role/aria-label 完整 + 8 项组件 spec + views/LibraryView.tsx 元数据行（行 870-890）插入 + 容器 flex-wrap 窄屏适配）
> 最后更新：2026-09-06（P0 续作：P0-2b outbox 接入 lifespan 启动 drain + 移除重复 /api/health 死路由 / P0-3b posters manifest 派生索引推广 / P0-2c service 层 outbox 改造 live+songs+export 全收口（tabs/presets/practice 正式结论不接）/ tsc --noEmit 假门废弃改 tsc -b + 测试文件纳入类型检查；P1 阶段二 a1-a4 四项补录路线图）
...
UI        ~99%       工作台/歌曲库（含垃圾桶 tab）/学歌/速查/海报/直播会话/复盘海报/数据统计（含「洞察」tab 点歌热度+最近演唱）/学习报告/专用海报/**今晚视图 P1-A1.1（默认首屏；复用 TonightWorkbench 5 区 + 屏幕标题 + 「进入工作台」CTA；navItems 第一项「今晚」Icon.tonight = 月相+星）/ 就绪度 4 徽章 P1-A1.2（ReadinessBadge 4 枚 chips：曲谱/歌词/音频/Key；LibraryView 元数据行内联可见；今晚视图 D 区共享同源 lib/readiness.ts）/命令面板（Cmd+K 全局找歌，含 eventsHeat 热度反哺排序）** + 4 嵌入点导出历史 + 弹唱视图（PlayView：v8.1 接入 <audio> + vocal/instrumental 切轨 + 进度联动 + R9 大字 Capo + 远观模式 + 再唱一遍 + 习惯 Capo） + MiniPlayer 全局底栏（M1.4：模式徽章 + 一键回弹唱） + 5 套 layout（grid/magazine/live-set/learning-report/fullscreen）+ 8 套主题（含月夜星河） + 5 嵌入 toast（M9.6b 删除撤销 + L1.1 4 kind + 状态变化） + ShortcutsPanel（?）+ Onboarding（首次启动）+ OnlineStatusBadge + StatusBar + HelpCenter（Cmd+Shift+?）+ **R5c WCAG AA 颜色合规（亮 muted-fg 5.19 / warning 5.31 / danger 5.50；--color-danger-text + --color-accent-text token；stagger reduce-motion 兜底；design-tokens.a11y.test.tsx 8 类对比度契约回归保护）** 均可用
```
...
| 测试 | pytest 95 passed（test_unit.py，含今晚 P0-2c service 层 outbox 改造全收口）+ 70 个 vitest 文件 849 passed（含今晚 R5c a11y 8 项 + P1-A1.1 TonightView 3 项 + P1-A1.2 readiness lib 12 项 + ReadinessBadge 8 项） + 5 套金标准 36/36（grid 16 + magazine 6 + live-set 5 + learning-report 5 + fullscreen 4，全部 0 像素差异）+ tsc -b 0 错 + OpenAPI 类型漂移/build | 当前 CI 质量门（2026-09-06 21:00 实测，今晚 P1 阶段二 4 刀收口后）；Windows 控制台 UTF-8 与依赖锁定已逐步收口 |
