...
>
> **下一步**：R8.2.x 弹唱录屏；M2.2 WebDAV 同步（凭证加密 + 拉/推/列表 + 自动同步策略）；M2.4 点歌条件；M2.12 网易云/QQ 榜单浏览 + 一键入库（netease `/api/toplist` 已就位 → 公开榜单列表 + 「一键加入曲库」）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R4 Runtime v2；R7 桌面正式发布门。
>
