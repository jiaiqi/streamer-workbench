...
> - **R4 Runtime v2 部分收口（V2.1 + V2.2 — 2026-08-12）** — 新增 `core/layouts/plan.py`（LayoutAnalysis/LayoutPlan/PagePlan/SectionPlan 5 个 frozen dataclass）+ `core/layouts/ctx.py`（LayoutContext）；4 套 layout `analyze(library, ctx)` 统一签名；`engine.render_pages` 解耦写死 magazine_flow import；magazine-flow `plan()` override 处理 axis/parameters；2 router 适配 v2 LayoutAnalysis；51 项新测试；1218 Python + 734 vitest 全过 + 16/16 金标准 0 像素差异 + 0 新依赖。**V2.3 Palette/Skin 接线 + V2.4 Parameters 注入 + V2.5 能力矩阵待推**。
...
>
> **下一步**：R4 Runtime v2 收口续（V2.3 Palette/Skin 接线 + V2.4 Parameters 注入 + V2.5 能力矩阵）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R7 桌面正式发布门。
>
