...
> **进度快照（2026-08-05 上午）**：R0–R8 + R4 Runtime v1 + R9 吉他手特化首批 + M9.6b 5 秒撤销 toast + M0 蓝图 v0.1 首批 + M1 本地最小可用首批 + M2.1 加密备份 AES-256 真加密 + **M2.2 WebDAV 同步**（零新依赖 stdlib urllib + xml.etree 客户端 + pyzipper AES 加密存 settings.json.webdav_config_encrypted + 6 端点 list/push/pull/test/config + SettingsView WebDavPanel 3 状态机 + 离线禁用 + 90 Python + 14 vitest）+ L1 体验打磨全 7 子项 + M2.5 综合洞察 + M2.6 错误全局 toast 化两批 + L2.1 批量操作第一批 + L2.2 批量导出 + L2.3 曲库导入导出 + M2.3 自动快照 + M2.7 在线元数据层骨架 + M2.8 NeteaseProvider 第一实现 + M2.9 LibraryView 在线补全 UI + M2.10 QQProvider 多源回退 + **M2.11 网易云/QQ 公开歌单导入**（PlaylistImportDialog + LibraryView 工具栏新按钮 + 16 vitest）+ **M2.12 榜单浏览 + 一键入库**（ChartsBrowseDialog + LibraryView 工具栏新按钮 + 12 vitest）+ **M2.13 macOS 桌面平台特性首批**（主进程播控菜单 + 系统通知 + Dock Badge 队列数 + PlayView ⇄ PlayerContext 双向同步；useSystemIntegration hook + 7 vitest；零新依赖）+ **M2.14 R3 回归 hotfix**（TheoryHelper isHover 块作用域 ReferenceError — 点学歌管理整个页面空白；改用 `hoverKey && COMMON_CHORDS_BY_KEY[hoverKey]` 直接判断 + 5 vitest 回归覆盖）+ **M2.16 海报分享**（跨平台剪贴板 + Finder 定位 + macOS 原生 Share Sheet — 3 IPC + osascript 桥接 NSSharingServicePicker；ExportDialog 完成态三按钮 + 非 macOS 平台 disabled；11 vitest）+ **M2.17 点歌条件**（4 字段 cooldown / max_queue / per_song / per_user，0=不限；QueueSnapshot 3 统计 + RequestPolicyService 4 检查 + LiveService 端到端联动 + GET/POST /api/live-sessions/{id}/policy + PolicyDialog UI；纯本地规则计算零外部 API 依赖；8 vitest）+ **R8.2.x 弹唱录屏首批**（观众号视角：全屏+系统音频；MediaRecorder VP9/Opus webm 容器；1GB 自动切片；PlayView 顶栏红点+计时器；RecordingDialog 4 状态机 idle/recording/paused/stopped/unsupported/error；LRC 实时事件 5s 限流推 SRT 字幕；useRecording hook 用 module-level store 共享 — 顶栏红点与 Dialog 同一份 state 零双订阅；preload 暴露 9 IPC listSources/start/pause/resume/appendLrc/stop/getState/listFiles/listSessions/delete；macOS 首次触发屏幕录制系统授权弹窗；存 data/recordings/{session-id}/；R8 弹唱不与 R0-R7 路径共用 — electron/recording/recorder.js 与 core/player/ 平行模块；33 Python 单元（Node 子进程驱动）+ 44 vitest；0 新外部依赖）全部收口 + **M3 海报 UI/UX 打磨 P0 收口**（后端：`/api/posters/{id}/thumb` 200x200 懒生成 + LANCZOS 缓存 `data/posters/{id}/.thumb.png` + mtime 失效 + PATCH /name + POST /duplicate + DELETE /api/export/jobs/{id} 取消；前端：PostersSidebar 重写 缩略图+搜索+排序+右键菜单+inline rename+撤销/重做图标+「×N 首·日期 时间」+「删除当前」；ExportDialog 三段式引导 idle/running/done/error，导出中可取消，失败可重试，完成态 6 动作分 2 行 + 再导一次；usePosterStore +rename +duplicate；684 vitest 全过 + tsc 0 错 + 16/16 金标准 0 像素差异 + 10 Python 新端点 + 50 vitest 新增） + **M3 海报 UI/UX 打磨 P1 收口**（快速预览放大镜：后端 `/thumb?size=200|400|600` 200 走磁盘缓存 + 400/600 内存即时放大；前端 hover 300ms 浮层 400x400 右上角 + mouseEnter/Leave + setTimeout 取消。批量操作：后端 `POST /api/posters/batch` action=delete/duplicate/set_theme + path traversal 防御 + 部分失败容错；前端多选模式 + checkbox + 工具栏 全选/清空/复制/🎨 主题/删除。`usePosterStore` +batch 自动 refreshList + 当前被删时 newDraft。useToast 无 Provider 静默 no-op。1058 pytest + 696 vitest 全过 + tsc 0 错 + 16/16 金标准 0 像素差异 + 0 新依赖） + **M2.4 WebDAV 自动同步**（让 M2.2 push/pull 真正落地，跨设备备份刚需）：后端 AutoSyncScheduler 后台 asyncio 循环 + 间隔 1-1440 分钟可配 + 方向 push/pull/both + 启动时根据 settings 决定启停 + 立即触发 + 失败不中断下个 tick + 每次写 last_at/last_status/last_error/last_remote_name 到 settings；3 端点 GET/POST auto-sync + run；webdav_sync.py push_internal/pull_internal/list_remote_internal/auto_run_once；主密码 base64 存 settings 关闭时清空；前端 WebDavPanel 自动同步 section（状态文本 + 间隔 7 档 + 方向 3 档 + 启用/关闭/立即同步 3 按钮 + 失败错误信息）。1069 pytest + 706 vitest 全过 + tsc 0 错 + 16/16 金标准 0 像素差异 + 0 新依赖。待推：M3 P2 拖拽排序 + ExportLogPanel 升级为可点历史抽屉 / Quick Look 预览 / M3 SQLite FTS5 / M4 Tauri 2 + PWA / R4 Runtime v2 / R7 桌面正式发布门。
>
...
> - **R4 Runtime v1 抽象** — `DataChannel` Literal 枚举（`song_library` / `live_session` / `learning_report`）；4 套 layout 显式声明 `supported_channels`，engine 不再 duck-typing；`get_layout(id, channel=...)` / `list_layouts(channel=...)` 按 channel 过滤；30 项测试 + 31/31 金标准复跑。
> - **R4 Runtime v2 全部收口（V2.1 + V2.2 + V2.3 + V2.4 + V2.5 — 2026-08-13）** — **V2.1** 新增 `core/layouts/plan.py`（LayoutAnalysis/LayoutPlan/PagePlan/SectionPlan 5 个 frozen dataclass）+ `core/layouts/ctx.py`（LayoutContext）；**V2.2** 4 套 layout `analyze(library, ctx)` 统一签名 + `engine.render_pages` 解耦写死 magazine_flow import + magazine-flow `plan()` override 处理 axis/parameters + 2 router 适配 v2 LayoutAnalysis；**V2.3** Palette/Skin 真实接线（`Palette.to_style()` + `Skin.from_palette_and_layout()` 工厂 + `Skin.apply_to_style()` + `DrawContext.effective_style` 双轨过渡 `skin > palette > style`）；**V2.4** Parameters 真正流到 `ctx.parameters`（`engine.render_page` 接受 `parameters` 可选参 + `render_document` 透传 + 3 套 layout 删除 `getattr(ctx, "parameters", {})` fallback 改用 `ctx.parameters` 直读）；**V2.5** 能力矩阵 + UI 灰显（`Theme.compatible_layouts` 字段 + `LayoutPlugin.compatible_themes()` 方法 + `core/layouts/compat.py` 4 函数 + 4 端点 `/api/compatibility` 系列 + 月夜星河 theme.json 演示字段 + grid_wrap override 排除月夜星河 + LayoutPicker 警告 banner）；95 项新测试（25 plan + 26 runtime_v2 + 21 palette_skin_wiring + 22 compatibility + 8 LayoutPicker.compat）；1261 Python + 742 vitest 全过 + 16/16 金标准 0 像素差异 + 0 新依赖。**R4 Runtime v2 全部收口**。
...
> - **M2.10 QQProvider 多源回退** — `core/metadata/providers/qq.py` 实现 QQ 音乐 4 个核心方法（`search` 用 `client_search_cp` + `t=0`；`get_song` 用 `fcg_play_single_song` 兼容 `mid/songmid` + `name/songname` + `album` 内/外字段；`get_lyric` 用 `fcg_query_lyric_new` + base64 解码 + 翻译字段；`get_similar` 用 search 兜底：`get_song → search by title → 排除自己 → top 5`）。`get_artist/get_album/get_playlist` 抛 `MetadataNotFound` 让 Router 跳下一个 provider；`get_charts` 返回 `[]`（不污染 errors）。业务码 `!= 0` → `MetadataUnavailable`；无 `code` 字段按成功处理（部分端点不返回）。`server/services/metadata.py` `build_default_router` 默认装 `[NeteaseProvider(), QQProvider()]`。37 provider 单元测试（含 2 个 Router 集成：netease 失败 → qq 接管；netease 成功 → qq 不调），验证 UnblockNeteaseMusic 多源回退架构真实工作。
> - **M2.11 网易云/QQ 公开歌单导入** — 新增 `ui/src/components/PlaylistImportDialog.tsx`（provider 选择 + playlist_id 输入 + 预览拉 `/api/metadata/playlist` + 歌曲列表默认全选 checkbox + 时长格式化 + 「导入选中」调 `/api/songs/import` merge + 结果展示 added/skipped/active/draft + 每首 notes 追加 `[meta:netease playlist=123 song_id=11 2026-08-04]` 标记来源 + status='draft'）。修改 `LibraryView.tsx` 工具栏新增「从歌单导入」按钮（垃圾桶 tab 时 disabled）。**不新增 server endpoint**：复用 M2.8 `/api/metadata/playlist` + M2.3 `/api/songs/import`。离线时预览按钮 disabled（L1.5 联动）。错误 toast（M2.6 useApiError）+ 行内双通道。16 项 PlaylistImportDialog 测试（open/关闭/preview/列表/全选切换/导入/结果展示/错误双通道/空歌单/offline 按钮）。`vitest 49 files / 564 tests passed`（548 → 564）。
> - **M2.12 榜单浏览 + 一键入库** — 新增 `ui/src/components/ChartsBrowseDialog.tsx`：打开自动调 `/api/metadata/charts` 拉官方榜单卡片（封面图 + 标题 + 描述；封面失败降级渐变色占位）；点榜单卡片 → 调 `/api/metadata/playlist { playlist_id: chart_id }` 拉榜内歌曲（歌曲列表默认全选 checkbox + 时长格式化 + 「导入选中」调 `/api/songs/import` merge + 结果展示 + 每首 notes 追加 `[meta:netease chart=19723756 song_id=a1 2026-08-04]` 标记来源 + status='draft'）。单面板钻取：榜单列表 ⇄ 榜内歌曲（返回不重新请求 charts；每个榜单歌曲详情缓存在 Map，返回再进不重复请求）。修改 `LibraryView.tsx` 工具栏新增「榜单浏览」按钮（垃圾桶 tab 时 disabled）。**不新增 server endpoint**：复用 M2.8 `/api/metadata/charts` + `/api/metadata/playlist` + M2.3 `/api/songs/import`。离线时不可加载（L1.5 联动）。错误 toast（M2.6 useApiError）+ 行内双通道。12 项 ChartsBrowseDialog 测试（auto-load charts / 榜单卡片 / charts 失败重试 / 钻取 playlist 默认全选 / 返回不重拉 charts / 榜单歌曲缓存 / 取消勾选 / toggle-all / 导入 merge + notes meta chart= + onImported / 结果展示 + 按钮已导入 / 空榜单 / 空歌曲 / open=false 不调 API）。`vitest 50 files / 576 tests passed`（564 → 576）。
> - **M2.13 macOS 桌面平台特性首批** — 主进程 `electron/main.js` 新增「播控」菜单（id=player-controls 含 ⏮/⏸-▶/⏭ + 队列数信息项，初始 disabled，`refreshPlayerMenu()` 根据 PlayerContext 状态实时 enable）+ `ipcMain.on("player:state")` 收渲染层 state（diff 驱动菜单/Dock badge/通知触发）+ `ipcMain.handle("player:notify")` 手动通知（30s 内同 tag 去重）+ `app.dock.setBadge(queueCount)`（仅 darwin）+ `new Notification(...)` 系统通知（点击切主窗口）。`electron/preload.js` 暴露 `streamer.sendPlayerState / onPlayerControl / notify / getPlayerState`（contextBridge 受 sandbox 保护）。`ui/src/hooks/useSystemIntegration.ts` 订阅 PlayerContext + LiveView 队列数（1Hz 节流 + JSON 去重），把 state 推主进程；接收主进程播控（play/pause → `setPlaying`；next/prev → 弹通知）。`ui/src/views/LiveView.tsx` 队列数变化时发 `live:queueCount` 事件 + 写 `window.__liveQueueCount`。`ui/src/play/PlayView.tsx` audio 元素 `play/pause/ended` 事件回写 PlayerContext + effect 监听 `playerCtx.isPlaying` 翻转去调 `audio.play/pause` + timeupdate 推 `setCurrentTime` → 主菜单/通知/未来 MediaSession 全可用。`ui/src/player/PlayerContext.tsx` `usePlayer` 在无 Provider 时降级 no-op（避免 PlayView 单测强绑 Provider）。`ui/vitest.config.ts` 接入 `src/hooks/*.test.tsx`。**零新依赖**（全用 Electron 内置 `Notification` / `app.dock` / `Menu`）。7 项 useSystemIntegration 测试 + 1 项 PlayerContext 合约更新。`vitest 51 files / 583 tests passed`（576 → 583）。
...
>
> **下一步**：R4 Runtime v2 全部收口后，按设计稿推动 R4 退出条件剩余 3 项（专用海报区产生日活观察 + 草稿/手动分页 v3 + 实际验证 grid-wrap 2 页）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R7 桌面正式发布门。
>
