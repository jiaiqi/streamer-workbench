...
>
> **下一步**：M2.4 自动同步策略（基于 M2.2 已收口的 push/pull + 定时任务）；Quick Look 预览（macOS 海报挑选体验）；M3 海报 UI/UX P1（缩略图快速预览放大镜 + 拖拽排序 + 批量操作）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R4 Runtime v2；R7 桌面正式发布门。
>
