...
>
> **下一步**：R8.2.x 弹唱录屏；M2.2 WebDAV 同步（凭证加密 + 拉/推/列表 + 自动同步策略）；M2.4 点歌条件；M2.8 NeteaseProvider 第一实现（基于 M2.7 协议拉网易云公开 search/song/lyric）；M2.9 LibraryView「在线补全」按钮 + 离线降级；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R4 Runtime v2；R7 桌面正式发布门。
>
