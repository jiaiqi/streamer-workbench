...
> - **M2.6 错误全局 toast 化第一批** — 新增 `useApiError` hook（`ui/src/async/useApiError.ts`）：`runWithToast(fn, label)` 自动 try/catch + 失败 `toast.error("${label}：${message}")` + 抛 `RequestFailure`（让上层 catch 仍可 `setError` → ErrorBanner）；AbortError 不弹 toast；无 ToastProvider 时 no-op（QuickView 独立窗口等场景）。`ToastContext` 导出便于 useContext 拿取；6 项新 vitest 覆盖成功/失败/无 label/Abort/RequestFailure 抛出/无 Provider。接入 LibraryView 3 处（seedSample / toggleStatus / deleteSong + 撤销的 restore）/ LiveView 6 处（loadDetail / handleCreate / handleClose / handleManualQueue / handleRecord / handleExportPoster）/ StatsView 4 处（Overview/Feed/Top/Distribution 加载 + Preset/海报创建）= 13 个 catch。第二批（SettingsView/LearningView/TonightSetCard/TrashView/PracticeLogDialog）待推。
...
>
> **下一步**：M2.6 错误全局 toast 化第二批（SettingsView/LearningView/TonightSetCard/TrashView/PracticeLogDialog）；R8.2.x 弹唱录屏；M2.2 WebDAV 同步（凭证加密 + 拉/推/列表 + 自动同步策略）；M2.3 自动快照；M2.4 点歌条件；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R4 Runtime v2；R7 桌面正式发布门；L2 工具箱完整性（批量操作 / 导入导出）。
>
