...
> - **R4 Runtime v2 部分收口（V2.1 + V2.2 + V2.3 + V2.4 — 2026-08-12）** — **V2.1** 新增 `core/layouts/plan.py`（LayoutAnalysis/LayoutPlan/PagePlan/SectionPlan 5 个 frozen dataclass）+ `core/layouts/ctx.py`（LayoutContext）；**V2.2** 4 套 layout `analyze(library, ctx)` 统一签名 + `engine.render_pages` 解耦写死 magazine_flow import + magazine-flow `plan()` override 处理 axis/parameters + 2 router 适配 v2 LayoutAnalysis；**V2.3** Palette/Skin 真实接线（`Palette.to_style()` + `Skin.from_palette_and_layout()` 工厂 + `Skin.apply_to_style()` + `DrawContext.effective_style` 双轨过渡 `skin > palette > style`）；**V2.4** Parameters 真正流到 `ctx.parameters`（`engine.render_page` 接受 `parameters` 可选参 + `render_document` 透传 + 3 套 layout 删除 `getattr(ctx, "parameters", {})` fallback 改用 `ctx.parameters` 直读）；72 项新测试（25 plan + 26 runtime_v2 + 21 palette_skin_wiring）；1239 Python + 734 vitest 全过 + 16/16 金标准 0 像素差异 + 0 新依赖。**V2.5 能力矩阵 + UI 灰显待推**。
...
>
> **下一步**：R4 Runtime v2 收口续（V2.5 能力矩阵 + UI 灰显）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R7 桌面正式发布门。
>
