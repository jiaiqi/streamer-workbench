...
>
> **下一步**：R8.2.x 弹唱录屏；M2.2 WebDAV 同步（凭证加密 + 拉/推/列表 + 自动同步策略）；M2.3 自动快照；M2.4 点歌条件；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R4 Runtime v2；R7 桌面正式发布门；L2 工具箱完整性（批量操作 / 导入导出）。
>
