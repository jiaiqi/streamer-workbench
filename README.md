...
> **进度快照（2026-08-03 凌晨）**：R0–R8 + R4 Runtime v1 + R9 吉他手特化首批 + M9.6b 5 秒撤销 toast + M0 蓝图 v0.1 首批 + M1 本地最小可用首批 + M2.1 加密备份 AES-256 真加密 + **L1 体验打磨首批**（toast 4 kind / 全局快捷键面板 `?` / 首次启动 Onboarding / 草稿保护）全部收口。待推：M2.2 WebDAV 同步 / M2.3 自动快照 / M2.4 点歌条件 / M2.5 洞察 / M3 SQLite FTS5 / M4 Tauri 2 / R8.2.x 录屏 / R4 Runtime v2 / R5c / R7 桌面正式发布门 / L1.5 离线检测。
>
...
>
> **下一步**：L1.5 离线检测（navigator.onLine + 顶栏状态点）；M2.2 WebDAV 同步（凭证加密 + 拉/推/列表 + 自动同步策略）；M3 SQLite FTS5 全文检索 + 学习看板 + 精力分级；M4 Tauri 2 桌面壳 + PWA；R8.2.x 录屏；R4 Runtime v2；R5c；R7 桌面正式发布门。
>
